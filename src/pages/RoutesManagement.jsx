...
export default function RoutesManagement() {
  ...
  const [activateLoading, setActivateLoading] = useState(null); // Track which route is being activated
  ...
  const fetchRoutes = async () => {
    try {
      ...
      // Using the specified endpoint to fetch routes
      ...
      setError(err.response?.data?.message || 'Failed to fetch routes. Please check your connection.');
      setRoutes([]); // Set empty routes array on error
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleActivate = async (routeId, isActive) => {
    try {
      setActivateLoading(routeId);
      
      // Use the specific activate route endpoint
  ...
  const handleFileUpload = async (routeId) => {
    ...
    if (!file.name.endsWith('.gpx') && !file.name.endsWith('.txt')) {
      alert('Please select a GPX file or text file with GPX data');
      return;
    }
    ...
    try {
      ...
      // Use the specific GPX upload endpoint
      ...
      if (response.data && response.data.data) {
        // Update the routes array with the updated route data from the response
        setRoutes(routes.map(route => {
                          ...
                          <li key={idx} className="py-2">
                            <div className="flex items-center">
                              ...
                              <span className="ml-auto text-xs text-gray-500">
                                {typeof checkpoint.distanceFromStart === 'number' 
                                  ? `${checkpoint.distanceFromStart.toFixed(1)} km` 
              ...
              <div className="border-t border-gray-200 px-4 py-4 sm:px-6 bg-gray-50">
                <div className="flex space-x-2">
                  ...
                  <button
                    ...
                    disabled={activateLoading === route._id}
